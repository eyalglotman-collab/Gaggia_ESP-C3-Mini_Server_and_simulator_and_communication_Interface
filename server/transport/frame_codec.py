"""Low-level framed packet codec for the simulator transport template."""
...
class MessageType(IntEnum):
    """@brief Enumerate the transport packet types used by the template.

    @details The simulator and future ESP32-C3 bridge firmware share this
    packet catalogue so command flow remains explicit and reviewable.
    """
...
class Frame:
    """@brief Describe one low-level framed packet.

    @details The frame carries host and device liveness counters along with a
    sequence number and a CRC-protected payload.
    """
...
# @details The low-level transport template keeps integrity checking in the
# framing layer so higher-level code only sees validated packets.
...
# @brief Encode a frame into bytes ready for serial transport.
# @details The encoded layout is SOF, type, payload length, host live integer,
# device live integer, sequence, payload, and CRC16.
...
# @brief Decode as many complete frames as possible from a mutable buffer.
# @details Incomplete trailing bytes remain in the buffer so the serial reader
# thread can append more data later without losing framing state.
