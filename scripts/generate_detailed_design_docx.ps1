...
$bodyItems = @(
    ...
    "Implementation Note: the current firmware source still carries legacy internal bridge enums such as BRIDGE_STATE_DISCONNECT and BRIDGE_STATE_ERROR, but the active host-visible transport contract is reset -> initialize -> connect -> keepalive with wait_for_com_reset handled by the Python supervisor."
    ...
    "  connect = wait for the ESP32-C3 bridge to report explicit client-connected or keepalive traffic"
    "  keepalive = active low-level supervision owned by the ESP32-C3 bridge after connect success"
    "  wait_for_com_reset = host-side latch that waits for explicit Reset Communication after repeated bridge-reported keepalive faults"
    "Primary State Diagram:"
    "  reset -> initialize -> connect -> keepalive"
    "  keepalive -> connect when the bridge reports transport loss"
    "  keepalive -> wait_for_com_reset after 5 sequential bridge-reported keepalive failures"
    "  wait_for_com_reset -> reset"
