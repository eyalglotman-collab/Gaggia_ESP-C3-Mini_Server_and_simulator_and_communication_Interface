...
$bodyItems = @(
    ...
    "  - before launching uvicorn, automatically stop any existing listener already bound to the requested simulator port"
    ...
    "  - stop any pre-existing process that is listening on the requested simulator port so only one backend instance remains authoritative"
    ...
    "  - scripts/run_simulator.ps1 = primary repo-local simulator launcher that resolves the repository root, prefers the local .venv interpreter, stops stale listeners on the target port, and starts uvicorn"
    "  - scripts/run_simulator.bat = Windows batch wrapper that forwards arguments into scripts/run_simulator.ps1 with execution-policy bypass and inherits the same stale-listener cleanup"
