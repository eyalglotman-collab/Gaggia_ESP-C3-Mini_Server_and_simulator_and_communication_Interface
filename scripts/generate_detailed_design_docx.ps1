...
$bodyItems = @(
    ...
    "  - the manual batch launcher shall verify the required local runtime components before it attempts any simulator startup script"
    "  - the required development-baseline runtime is Python 3.13.3 with fastapi 0.135.1, uvicorn 0.41.0, and pyserial 3.5 in the repository-local .venv"
    ...
    "  - scripts/verify_simulator_installation.ps1 = manual-launch preflight check that verifies required runtime components and exact development versions before startup"
    "  - scripts/launch_simulator_ui.ps1 = manual-launch helper that starts the backend host, waits for /health, and shows a browser-open confirmation message box"
    "  - scripts/run_simulator.bat = Windows batch wrapper that forwards arguments into scripts/launch_simulator_ui.ps1 with execution-policy bypass"
