...
$bodyItems = @(
    ...
    "  - LinkRuntime.reset() = clear counters and watchdog state, send RESET, and return to reset baseline"
    "  - LinkRuntime.initialize() = validate mirrored configuration, send INITIALIZE, auto-send CONNECT, and wait for the ESP connect response"
    ...
    "  error"
    "State Semantics:"
    "  reset = hard reset, counter clear, buffer clear, and ESP controller re-initialization"
    "  initialize = load the ESP controller with mirrored Wi-Fi and server constants, then auto-start connect"
