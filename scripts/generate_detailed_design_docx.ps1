...
$bodyItems = @(
    ...
    ""
    "14. Protocol Contract and Handshake Design"
    "Purpose: this chapter defines the generic LCD-controller transport protocol contract so both endpoints can negotiate capabilities and schema safely while keeping runtime packets compact."
    "Protocol Name: GLCP (Gaggia LCD-Controller Protocol)."
    "Primary Scope: endpoint negotiation, frame integrity, schema compatibility, and deterministic handshake behavior for controller <-> user-side transport."
    ""
    "14.1 Reference Patterns Used for This Chapter"
    "Design pattern source A (TCP, RFC 9293): explicit state progression for connection setup and confirmation before data exchange."
    "Design pattern source B (QUIC, RFC 9000): version negotiation and explicit handling of unsupported versions before normal traffic is accepted."
    "Design pattern source C (gRPC over HTTP/2 protocol description): strict wire framing plus an explicit request/response sequencing contract and keepalive ping behavior."
    "Design pattern source D (Protocol Buffers compatibility guidance): preserve backward compatibility through additive fields, stable identifiers, and strict no-reuse rules for retired identifiers."
    ""
    "14.2 Protocol Goals and Non-Goals"
    "Goals:"
    "  - deterministic startup negotiation with clear reject reasons"
    "  - compatibility across independent build cycles via schema hash negotiation"
    "  - strict frame integrity checks before payload parsing"
    "  - compact data-plane packets after successful handshake"
    "Non-Goals:"
    "  - dynamic shipping of C headers over the wire"
    "  - parser-side dependence on compiler packing/alignment rules"
    "  - acceptance of unsupported major versions"
    ""
    "14.3 Wire Frame Contract (GLCP v1)"
    "Field Layout (all integers little-endian):"
    "  Offset | Size | Field | Notes"
    "  0 | 2 | sof | fixed 0xA55A"
    "  2 | 1 | major | protocol major version"
    "  3 | 1 | minor | protocol minor version"
    "  4 | 2 | msg_id | message identifier"
    "  6 | 2 | flags | bitmask: ack_required, is_response, is_error"
    "  8 | 4 | session_id | selected by server/bridge after handshake"
    "  12 | 4 | seq | monotonic sequence in active session"
    "  16 | 2 | payload_len | number of payload bytes"
    "  18 | 4 | payload_crc32 | CRC32(IEEE reflected) over payload only"
    "  22 | N | payload | message body bytes"
    "Validation Rule: reject frame if SOF mismatch, unsupported major version, length mismatch, or payload CRC mismatch."
    ""
    "14.4 Message Families"
    "Control Plane:"
    "  - HELLO (0x0001): peer advertises version window, capabilities, and schema hash set"
    "  - HELLO_ACK (0x0002): server decision (accept/reject), selected protocol version, selected schema hash"
    "  - NACK (0x0003): structured rejection for invalid or incompatible handshake inputs"
    "  - SCHEMA_GET (0x0010): peer requests descriptor for unknown selected schema hash"
    "  - SCHEMA_CHUNK (0x0011): chunked descriptor transfer"
    "  - PING (0x0020) / PONG (0x0021): liveness and RTT measurement"
    "Data Plane:"
    "  - SYSTEM_STATE (0x0100)"
    "  - SENSOR_SNAPSHOT (0x0101)"
    "  - SHOT_SNAPSHOT (0x0102)"
    "  - PROFILE_GET/SET/DATA (0x0200/0x0201/0x0202)"
    "  - SETTINGS_GET/SET/DATA (0x0210/0x0211/0x0212)"
    "  - EVENT_RAISED (0x0300)"
    "  - COMMAND_APPLY (0x0301)"
    ""
    "14.5 Capability and Schema Negotiation"
    "HELLO fields:"
    "  - max_frame_size"
    "  - heartbeat_ms"
    "  - build_id"
    ...
    "  - supported_major_min"
    "  - supported_major_max"
    "  - schema_hash_count + schema_hashes[]"
    "HELLO_ACK fields:"
    "  - accepted (0 or 1)"
    "  - selected_major and selected_minor"
    "  - server_max_frame_size"
    "  - heartbeat_ms"
    "  - server_build_id"
    "  - capability_bits"
    "  - selected_schema_hash"
    "Selection rule: choose first common schema hash according to server preference order. If none exists, return reject with no-common-schema reason."
    ""
    "14.6 Compatibility Contract"
    "Major Version Rule: mismatch is hard-fail at handshake."
    "Minor Version Rule: same major allows forward-compatible operation subject to capability checks."
    "Schema Rule: runtime data messages are valid only after schema hash selection succeeds."
    "Field Evolution Rule (best-practice alignment):"
    "  - add new fields as optional/additive only"
    "  - never change meaning of existing field identifiers"
    "  - never reuse retired field identifiers"
    "  - ignore unknown optional fields safely"
    ""
    "14.7 Sequence Diagram A - Nominal Handshake (Known Schema)"
    "Figure 14-A:"
    "  Peer A (Client/UI)            Peer B (Controller/Bridge)"
    "        |                                   |"
    "        | HELLO(ver range, caps, hashes)    |"
    "        |---------------------------------->|"
    "        |                                   | validate + select schema"
    "        | HELLO_ACK(accepted=1, schema=X)   |"
    "        |<----------------------------------|"
    "        | SETTINGS_GET / PROFILE_GET / ...  |"
    "        |---------------------------------->|"
    ...
    "        |<----------------------------------|"
    ""
    "14.8 Sequence Diagram B - Unknown Schema Then Descriptor Fetch"
    "Figure 14-B:"
    "  Peer A (Client/UI)            Peer B (Controller/Bridge)"
    "        |                                   |"
    "        | HELLO(hashes do not include server preferred hash)"
    "        |---------------------------------->|"
    "        | HELLO_ACK(accepted=1, schema=Y)   |"
    "        |<----------------------------------|"
    "        | SCHEMA_GET(schema=Y)              |"
    "        |---------------------------------->|"
    "        | SCHEMA_CHUNK(1/N)                 |"
    ...
    "        | SCHEMA_CHUNK(N/N)                 |"
    "        |<----------------------------------|"
    "        | cache descriptor, activate parser |"
    "        | normal data-plane traffic starts  |"
    ""
    "14.9 Sequence Diagram C - Reject Path (No Common Major Version)"
    "Figure 14-C:"
    "  Peer A                             Peer B"
    "    | HELLO(major range excludes server major) |"
    "    |------------------------------------------>|"
    "    | HELLO_ACK(accepted=0) or NACK            |"
    "    |<------------------------------------------|"
    "    | enter error/recovery path                 |"
    "    | do not send data-plane messages           |"
    ""
    "14.10 Handshake Examples (Concrete Values)"
    "Example 1 - HELLO (decoded view):"
    "  major=1 minor=0 msg_id=0x0001 session_id=0 seq=1"
    "  payload.max_frame_size=2300"
    "  payload.heartbeat_ms=300"
    "  payload.capability_bits=0x000000000000000F"
    "  payload.supported_major_min=1"
    "  payload.supported_major_max=1"
    "  payload.schema_hashes=[0x9B9F1264A2F0CC11, 0x8C2E3A71D55B44F0]"
    "Example 2 - HELLO_ACK accept:"
    "  msg_id=0x0002 flags=is_response accepted=1"
    "  selected_major=1 selected_minor=0"
    "  selected_schema_hash=0x9B9F1264A2F0CC11"
    "Example 3 - Reject:"
    "  msg_id=0x0003 flags=is_error"
    "  reason=no_common_schema or unsupported_major_version"
    ""
    "14.11 Error Handling and Recovery Rules"
    "Frame Error Rule: malformed frame increments transport error counter and may trigger session reset after threshold."
    "Negotiation Error Rule: rejected HELLO does not consume or alter active session counters."
    "Runtime Parse Error Rule: payload parse failures are isolated to the message; repeated parse failures trigger link health downgrade."
    "Timeout Rule: missing PONG or keepalive response beyond watchdog window triggers reconnect workflow."
    ""
    "14.12 Security and Robustness Baseline"
    "Integrity: payload CRC32 is mandatory for every frame."
    "Replay/Mixup Guard: session_id plus monotonic seq prevents stale-frame acceptance after reconnect."
    "Downgrade Guard: selected major/minor and schema hash are explicit in HELLO_ACK and must be checked before state acceptance."
    "Input Hardening: unknown msg_id without negotiation support returns NACK and is never treated as application data."
    ""
    "14.13 Implementation Mapping in This Repository"
    "Wire schema API: firmware/esp32c3_bridge/main/ProtocolLCD_Controller.h"
    "Wire codec and negotiation helpers: firmware/esp32c3_bridge/main/ProtocolLCD_Controller.c"
    "State contract model: firmware/esp32c3_bridge/main/SystemStatesLCD_Controller.h"
    "Profile/settings data contract: firmware/esp32c3_bridge/main/DataStructuresLCD_Controller.h"
    "Integration status: phase-1 codec and negotiation helpers are implemented; runtime bridge handshake routing is the next phase."
    ""
    "14.14 Acceptance Checklist for Protocol Integration"
    "  - HELLO and HELLO_ACK roundtrip passes with matching schema hash"
    "  - reject path verified for unsupported major version"
    "  - reject path verified for empty schema intersection"
    "  - CRC mismatch causes deterministic frame rejection"
    "  - unknown optional field does not break parser"
    "  - PING/PONG RTT path produces bounded latency samples"
    "  - reconnect establishes a new session_id and resets sequence origin"
    ""
    "14.15 External Reference URLs"
    "  - TCP transport-state reference (RFC 9293): https://www.rfc-editor.org/rfc/rfc9293"
    "  - QUIC version-negotiation reference (RFC 9000): https://www.rfc-editor.org/rfc/rfc9000"
    "  - gRPC wire protocol reference (HTTP/2 mapping): https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-HTTP2.md"
